...
import os, re, hmac, json, time, uuid, asyncio, hashlib, urllib.request
...
async def refer(request: Request):
    ...
    jid = job_new(u["id"], "refer", len(accs), {"link": b["bot_link"], "method": b.get("method", "no_captcha")})
...
async def channels(request: Request):
    ...
    jid = job_new(u["id"], "channels", len(accs), {"action": b.get("action", "join"), "channels": chans})
...
async def message(request: Request):
    ...
    jid = job_new(u["id"], "message", len(accs), {"target": b["target"]})
